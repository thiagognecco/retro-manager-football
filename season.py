...
class SeasonManager:
    ...
    def apply_match_to_standings(match_record: Dict, standings: Dict[str, Dict]):
        ...
        Atualiza jogos disputados, vitórias, empates, derrotas, gols e saldo.
        """
    ...
    def get_sorted_standings(standings: Dict[str, Dict]) -> List[Dict]:
        """Retorna lista ordenada por pontos, saldo de gols, gols pró, vitórias e nome como último critério."""
        table = list(standings.values())
        # Ordem de desempate: pontos, saldo de gols (gd), gols pró (gf), vitórias, nome
        table.sort(key=lambda s: (-s['points'], -s['gd'], -s['gf'], -s['wins'], s['team']))
        return table
