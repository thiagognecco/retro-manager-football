import math
import random

# Simple player AI for visual match mode.
# API: update_player(player, ball, teammates, opponents, dt)
# player: dict with keys 'game_player' (database.Player), 'x','y','vx','vy','team', 'has_ball'
# ball: dict with keys 'x','y','vx','vy','possession' (may be player dict or None)
# teammates/opponents: lists of player dicts
# dt: seconds since last frame


def _distance(a_x, a_y, b_x, b_y):
    return math.hypot(a_x - b_x, a_y - b_y)


def _normalize(dx, dy):
    d = math.hypot(dx, dy)
    if d == 0:
        return 0.0, 0.0
    return dx / d, dy / d


def update_player(player, ball, teammates, opponents, dt):
    """
    Update player's velocity (vx, vy) and optionally interact with the ball.
    Mutates `player` and `ball` in-place.
    Implements seek + arrive steering, and a simple stamina system based on player.fis.
    """
    gp = player.get('game_player')
    if gp is None:
        return

    # Ensure velocity and stamina exist
    player.setdefault('vx', 0.0)
    player.setdefault('vy', 0.0)
    if 'stamina' not in player:
        # stamina range ~0.2 .. 1.0 based on physical (fis)
        player['stamina'] = max(0.2, min(1.0, gp.fis / 5.0))

    stamina = player['stamina']

    # Movement parameters
    base_speed = 80 + (gp.fis * 20)  # pixels per second
    max_speed = base_speed * 1.6

    # Helper to apply stamina effects and recover/drain
    def _apply_stamina(cur_speed, dt):
        exertion = min(1.0, abs(cur_speed) / max_speed)
        # drain faster when exerting
        drain = exertion * 0.05 * dt
        player['stamina'] = max(0.0, player['stamina'] - drain)
        # passive recovery when low exertion
        if exertion < 0.15:
            player['stamina'] = min(1.0, player['stamina'] + 0.08 * dt)

    # If player currently has the ball, decide action: shoot, pass, or dribble
    if player.get('has_ball'):
        dec = getattr(gp, 'dec', 1)
        tec = getattr(gp, 'tec', 1)
        shoot_bias = 0.15 + ((dec + tec) / 10.0) * 0.45
        if gp.star:
            shoot_bias += 0.05

        # Determine which direction is opponent goal based on team
        if player.get('team') == 1:
            goal_x = 800
        else:
            goal_x = 0

        dist_to_goal = abs(goal_x - player['x'])
        near_goal = dist_to_goal < 220

        # Prefer shooting if near goal
        if near_goal and random.random() < shoot_bias:
            dirx, diry = _normalize(goal_x - player['x'], 300 - player['y'])
            power = 320 + (tec * 90)
            # stamina reduces shot power slightly
            power *= (0.6 + 0.4 * player['stamina'])
            ball['vx'] = dirx * power
            ball['vy'] = diry * power * 0.6
            player['has_ball'] = False
            ball['possession'] = None
            _apply_stamina(power, dt)
            return

        # Try to pass to a forward teammate
        forward_teammates = [t for t in teammates if t is not player]
        if forward_teammates:
            if player.get('team') == 1:
                target = max(forward_teammates, key=lambda t: (t['x'], -abs(t['y'] - player['y'])))
            else:
                target = min(forward_teammates, key=lambda t: (t['x'], abs(t['y'] - player['y'])))

            dirx, diry = _normalize(target['x'] - player['x'], target['y'] - player['y'])
            power = 220 + (tec * 70)
            power *= (0.6 + 0.4 * player['stamina'])

            # slight inaccuracy based on decision attribute
            inaccuracy = max(0.0, (5 - gp.dec) * 0.02)
            if random.random() < inaccuracy:
                dirx += (random.random() - 0.5) * 0.3
                diry += (random.random() - 0.5) * 0.3
                dirx, diry = _normalize(dirx, diry)

            ball['vx'] = dirx * power
            ball['vy'] = diry * power * 0.6
            player['has_ball'] = False
            ball['possession'] = None
            _apply_stamina(power, dt)
            return

        # Dribble: move toward opponent goal while keeping the ball
        if player.get('team') == 1:
            dirx, diry = 1.0, 0.0
        else:
            dirx, diry = -1.0, 0.0
        speed = base_speed * 0.6 * (0.6 + 0.4 * player['stamina'])
        player['vx'] = dirx * speed
        player['vy'] = diry * speed * 0.4
        _apply_stamina(speed, dt)
        return

    # If player doesn't have the ball, move towards it (seek with arrive)
    bx, by = ball.get('x', 0.0), ball.get('y', 0.0)
    dx, dy = bx - player['x'], by - player['y']
    dist = _distance(player['x'], player['y'], bx, by)

    # Attempt to pick up free ball if close
    if dist < 18 and ball.get('possession') is None:
        take_prob = 0.5 + ((gp.fis + gp.dec) - 2) * 0.06
        if random.random() < take_prob:
            player['has_ball'] = True
            ball['possession'] = player
            ball['vx'] = 0.0
            ball['vy'] = 0.0
            return

    # Pursuit: seek with arrival
    if dist > 6:
        arrive_radius = 50.0
        nx, ny = _normalize(dx, dy)
        desired_speed = base_speed * (0.6 + (gp.fis / 10.0))
        # slow down when arriving
        if dist < arrive_radius:
            desired_speed *= max(0.2, (dist / arrive_radius))
        # stamina reduces available speed
        desired_speed *= (0.5 + 0.5 * player['stamina'])
        player['vx'] = nx * desired_speed
        player['vy'] = ny * desired_speed
        _apply_stamina(desired_speed, dt)
    else:
        # small jitter/hold position
        player['vx'] *= 0.5
        player['vy'] *= 0.5
        # recover some stamina when idle
        player['stamina'] = min(1.0, player['stamina'] + 0.06 * dt)

    # If opponent has the ball, and this player is close, attempt tackle
    poss = ball.get('possession')
    if poss and poss in opponents:
        dist2 = _distance(player['x'], player['y'], poss['x'], poss['y'])
        if dist2 < 22:
            tackle_prob = 0.25 + (gp.fis / 10.0)
            if random.random() < tackle_prob:
                player['has_ball'] = True
                poss['has_ball'] = False
                ball['possession'] = player
                ball['vx'] = 0.0
                ball['vy'] = 0.0
                # knock-back for opponent
                dx2 = poss['x'] - player['x']
                dy2 = poss['y'] - player['y']
                poss['vx'] = -dx2 * 2
                poss['vy'] = -dy2 * 2
                return
